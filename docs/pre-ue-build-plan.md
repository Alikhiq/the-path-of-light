...
- **Reference set** — the 5-tier render ladder + "Lamplight Realism" target renders (`docs/render-tiers-and-v1-target.md`), real Abbasid Baghdad architecture + period dress, āyāt-engine screenshots + the one-page **look bible** (grounded night realism: baked lighting, 2K PBR, fog, palette; what the aura + Maʿṣūm-presence do and never do — `docs/art-bible-faces-and-auras.md`).
- **Pipeline-test spec — DONE ✓ (2026-07-12, PASSED).** The MCP loop is proven: agent-built greybox courtyard, voiced NPC via Blueprint, materials, PIE — all with zero GUI. It was built in the now-retired PS1 look as a throwaway tech test. The **real build** follows `docs/lamplight-realism-4060-build-spec.md` (Lamplight Realism, real human NPC, ~30–60 fps on the 4060). Next real milestone = a vertical-slice courtyard in the Lamplight look per that spec.
